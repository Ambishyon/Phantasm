...
PartCacheStatic.__type = "ObjectCache" -- For compatibility with TypeMarshaller
...
local function MakeFromTemplate(template: Instance, currentCacheParent: Instance): Instance
	local part: Instance = template:Clone()
	...
	elseif part:IsA"Attachment" then
		part.WorldCFrame = CF_REALLY_FAR_AWAY
	else
		part.Parent = nil
	end
...
function PartCacheStatic.new(template: Instance, numPrecreatedObjects: number?, currentCacheParent: Instance?): PartCache
	local newNumPrecreatedObjects: number = numPrecreatedObjects or 5
	...
	for _ = 1, newNumPrecreatedObjects do
...
function PartCacheStatic:GetObject(): Instance
	assert(getmetatable(self) == PartCacheStatic, ERR_NOT_INSTANCE:format("GetObject", "ObjectCache.new"))
	
	if #self.Open == 0 then
		warn("No parts available in the cache! Creating [" .. self.ExpansionSize .. "] new object instance(s) - this amount can be edited by changing the ExpansionSize property of the PartCache instance... (This cache now contains a grand total of " .. tostring(#self.Open + #self.InUse + self.ExpansionSize) .. " objects.)")
...
function PartCacheStatic:ReturnObject(part: Instance)
	assert(getmetatable(self) == PartCacheStatic, ERR_NOT_INSTANCE:format("ReturnObject", "ObjectCache.new"))
...
function PartCacheStatic:SetCacheParent(newParent: Instance)
	assert(getmetatable(self) == PartCacheStatic, ERR_NOT_INSTANCE:format("SetCacheParent", "ObjectCache.new"))
...
function PartCacheStatic:Expand(numParts: number): ()
	assert(getmetatable(self) == PartCacheStatic, ERR_NOT_INSTANCE:format("Expand", "ObjectCache.new"))
...
function PartCacheStatic:Dispose()
	assert(getmetatable(self) == PartCacheStatic, ERR_NOT_INSTANCE:format("Dispose", "ObjectCache.new"))
	for i = 1, #self.Open do
		self.Open[i]:Destroy()
	end
	for i = 1, #self.InUse do
